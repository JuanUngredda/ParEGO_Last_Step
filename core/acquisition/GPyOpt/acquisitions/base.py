...
class AcquisitionBase(object):
    ...
    def __init__(self, model, space, optimizer,NSGA_based=False, alpha =None, model_c=None ,cost_withGradients=None):
    ...
    def acquisition_function(self,x):
        ...
        return -np.array(f_acqu).reshape(-1)*np.array(self.space.indicator_constraints(x)).reshape(-1)#/cost_x f_acqu*self.space.indicator_constraints(x) #

    def acquisition_function_constrained(self):
        recommended_x, f_acqu = self._compute_acq_constraints()
        return recommended_x, np.array(f_acqu).reshape(-1)

    def _compute_mu(self,x):
        f_acqu = self._compute_mu(x)
        cost_x, _ = self.cost_withGradients(x)
        return -(f_acqu*self.space.indicator_constraints(x))
    ...
    def _compute_Fz(self,x):
        f_acqu = self._compute_Fz(x)
        cost_x, _ = self.cost_withGradients(x)

        return -(f_acqu*self.space.indicator_constraints(x))

    def _compute_Fz_xopt_withGradients(self,x):
        f_acqu = self._compute_Fz_xopt_withGradients(x)
        cost_x, _ = self.cost_withGradients(x)
    ...
    def acquisition_Gradients(self, x):
        ...
        f_acqu, df_acq_cost = self._compute_acq_withGradients(x)
        return df_acq_cost*self.space.indicator_constraints(x) #df_acq_cost*self.space.indicator_constraints(x) # #


    def optimize(self, duplicate_manager=None, re_use=False, constrained=False):
        """
        Optimizes the acquisition function (uses a flag from the model to use gradients or not).
        """
        if not self.analytical_gradient_acq:

            if constrained and self.NSGA_based:

                out = self.acquisition_function_constrained()
            else:

                out = self.optimizer.optimize(f=self.acquisition_function, duplicate_manager=duplicate_manager, re_use=re_use)
        else:
            # self._gradient_sanity_check_2D(self.acquisition_function, self.acquisition_Gradients, delta=1e-6)
            #self._gradient_sanity_check_2D_TEST2(self.acquisition_function_withGradients, delta=1e-6)
    ...
    def _gradient_sanity_check_2D(self, f, grad_f, x_value = None, delta=1e-4):
        initial_design = np.random.random((80,2))*(np.array([0, 5.0])- np.array([0.0, 3.0])) + np.array([0, 3.0])# self.test_samples
        fixed_dim =0
        variable_dim = 1
        ...
        if x_value is None:
            ...
            print("FD", np.array(f_delta / (2 * delta)).reshape(-1), "analytical",
                  analytical_gradient_val.reshape(-1))
            analytical_grad.append(analytical_gradient_val.reshape(-1))
        func_val = np.array(func_val)
    ...
    def _compute_acq_constraints(self,x):
