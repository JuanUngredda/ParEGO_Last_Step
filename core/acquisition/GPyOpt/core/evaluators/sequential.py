...
class Sequential(EvaluatorBase):
    ...
    def compute_batch(self, duplicate_manager=None,context_manager=None, re_use = False, constrained=False):

        """
        Selects the new location to evaluate the objective.
        """
        #print("compute batch")
        x, _ = self.acquisition.optimize(duplicate_manager=duplicate_manager, re_use = re_use, constrained=constrained) #previusly just optimize
