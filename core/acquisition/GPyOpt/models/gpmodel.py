...
class GPModel(BOModel):
    ...
    def _create_model(self, X, Y):
        ...
        if self.kernel is None:
            kern = GPy.kern.Matern32(self.input_dim)
            print("matern kernel")
        else:
            kern = self.kernel
            self.kernel = None
        ...
        if self.exact_feval:
            ...
            self.model.rbf.variance.constrain_bounded(0, np.std(Y), warning=False)
            # self.model.rbf.lengthscale.constrain_fixed(24.44, warning=False)
            #self.model.Gaussian_noise.constrain_fixed(self.noise_var, warning=False)
            #self.model.Gaussian_noise.constrain_bounded(1e-21,5)
            self.model.rbf.lengthscale.constrain_bounded((np.max(X[:,0])- np.min(X[:,0]))*0.10,np.max(X[:,0])*0.25)
