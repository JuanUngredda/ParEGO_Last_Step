...
class OptLbfgs(Optimizer):
    ...
    def optimize(self, x0, f=None, df=None, f_df=None):
        ...
        print("x0", x0, "result_x", res[0], "res[1]",res[1])
...
class NSGA(Optimizer):
    ...
    def __init__(self, bounds, maxiter=20):
        super(NSGA, self).__init__(bounds)
        self.maxiter = maxiter
    ...
    def optimize(self, x0=None, f=None, df=None, f_df=None):
        ...
        if len(f) <2:
            print("Include two or more objectives")
            raise

        bounds = self.bounds_format_adapter(bounds=self.bounds)

        udp = IOP(f=f, bounds=bounds)
...
def apply_optimizer(optimizer, x0=None, f=None, df=None, f_df=None, duplicate_manager=None, context_manager=None, space=None):
    """
...
def choose_optimizer(optimizer_name, bounds):
        ...
        elif optimizer_name == "NSGA":
            optimizer = NSGA(bounds)
        else:
