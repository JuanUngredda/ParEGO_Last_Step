import numpy as np
import GPyOpt
from GPyOpt.objective_examples.experiments2d import POL
import GPy as GPy
...
def POL_function_caller_test(rep):
    ...
    np.random.seed(rep)
    folder = "RESULTS"
    subfolder = "POL_EGO_utility_LS"
    cwd = os.getcwd()
    path = cwd + "/" + folder + "/"+subfolder

    # func2 = dropwave()
    POL_func= POL(sd=np.sqrt(noise))
    ...
    space =  GPyOpt.Design_space(space =[{'name': 'var_1', 'type': 'continuous', 'domain': (-np.pi, np.pi)},{'name': 'var_2', 'type': 'continuous', 'domain': (-np.pi, np.pi)}])#GPyOpt.Design_space(space =[{'name': 'var_1', 'type': 'continuous', 'domain': (0,100)}])#
    n_f = 2
    n_c = 0
    input_d = 2
    m =n_f
    ...
    acq_opt = GPyOpt.optimization.AcquisitionOptimizer(optimizer='lbfgs', space=space, model=model_f, model_c=None)
    ...
    acquisition = HVI(model=model_f, model_c=None , alpha=alpha, space=space, NSGA_based=True,optimizer = acq_opt, utility= U, true_func=f)

    last_step_acquisition = Last_Step(model_f=model_f, model_c=None , true_f=f, true_c=None,n_f=n_f, n_c=n_c, acquisition_optimiser = acq_opt, acquisition_f=acquisition,seed=rep, space=space, path=path)
    ...
    X, Y, C, Opportunity_cost = bo.run_optimization(max_iter = max_iter,  rep=rep, last_step_evaluator=last_step_acquisition, path=path, verbosity=False)
    print("Code Ended")
...
POL_function_caller_test(rep=7)
print("ready")
