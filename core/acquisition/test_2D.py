import numpy as np
import GPyOpt
from GPyOpt.objective_examples.experiments2d import mistery,  test_function_2, new_brannin, f1_Binh, POL, TNK
import GPy as GPy
...
def function_caller_test(rep):
    ...
    space =  GPyOpt.Design_space(space =[{'name': 'var_1', 'type': 'continuous', 'domain': (0,1.5)},{'name': 'var_2', 'type': 'continuous', 'domain': (0,1.5)}])#GPyOpt.Design_space(space =[{'name': 'var_1', 'type': 'continuous', 'domain': (0,100)}])#
    n_f = 2
    n_c = 2
    input_d = 2
    m =n_f
    ...
    # --- Initial design
    #initial design
    initial_design = GPyOpt.experiment_design.initial_design('latin', space, 15)#2*(input_d+1))
    ...
    last_step_acquisition = Last_Step_KG(model=model_f, model_c=model_c , alpha=alpha, space=space, optimizer = acq_opt, utility= U, true_func=f)
    evaluator = GPyOpt.core.evaluators.Sequential(acquisition)
    last_step_evaluator = GPyOpt.core.evaluators.Sequential(last_step_acquisition)
    bo = BO(model_f, model_c, space, f, c, acquisition, evaluator, initial_design,  ref_point=ref_point)


    max_iter  = 35
    # print("Finished Initialization")
    X, Y, C, Opportunity_cost = bo.run_optimization(max_iter = max_iter, last_step_evaluator=last_step_evaluator, verbosity=True)
    print("Code Ended")
