...
class multi_outputGP(object):
    ...
    def posterior_mean_builder(self, X, j):
        """
        Predictions with the model. Returns posterior mean at X.
        """
        X = np.atleast_2d(X)

        return self.output[j].posterior_mean(X)[:, 0]

    def posterior_var_builder(self, X, j, noise=False):
        """
        Returns posterior variance at X of model j.
        """
        X = np.atleast_2d(X)
        return self.output[j].posterior_variance(X, noise=noise)[:, 0]

    def posterior_mean_grad_builder(self,  X, j):
        """
        Computes dmu/dX(X).
        :param X:  input observations
        """

        dmu_dX_j = self.output[j].posterior_mean_gradient(X)

        return dmu_dX_j

    def posterior_variance_grad_builder(self, X, j):
        """
        Computes dmu/dX(X).
        :param X:  input observations
        """

        dvar_dX_j = self.output[j].posterior_variance_gradient(X)
        return dvar_dX_j
    ...
    def posterior_var_gradient_f(self,X):
        ...
        :return:
        """
        var_df_dX = np.empty((self.output_dim, X.shape[0], X.shape[1]))
        for j in range(self.output_dim):
            tmp = self.output[j].posterior_variance_gradient_var(X)
            var_df_dX[j, :, :] = tmp[:, :, 0]
        return var_df_dX
