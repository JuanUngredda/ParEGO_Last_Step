...
def POL_function_caller_test(rep):
    ...
    m = 2
    Overall_Budget = 100
    for LS_budget in list(range(1, 99, 10))[1:]:
        np.random.seed(rep)
        ...
        folder = "RESULTS"
        subfolder = "POL_ParEGO_Main_" + str(Main_Alg_Budget) + "_" + "Last_Step_Budget_" + str(Last_Step_Budget)
        cwd = os.getcwd()
        ...
        space = GPyOpt.Design_space(space=[{'name': 'var_1', 'type': 'continuous', 'domain': (-np.pi, np.pi)},
                                           {'name': 'var_2', 'type': 'continuous', 'domain': (-np.pi,
                                                                                              np.pi)}])  # GPyOpt.Design_space(space =[{'name': 'var_1', 'type': 'continuous', 'domain': (0,100)}])#
