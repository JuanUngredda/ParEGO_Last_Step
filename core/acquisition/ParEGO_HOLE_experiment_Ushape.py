import numpy as np
import GPyOpt
from GPyOpt.objective_examples.experiments2d import HOLE
import GPy as GPy
...
def HOLE_function_caller_test(rep):
    ...
    for LS_budget in [0, 1, 3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 36, 39]:
        ...
        folder = "RESULTS"
        subfolder = "HOLE_ParEGO_Main_" + str(Main_Alg_Budget) + "_" + "Last_Step_Budget_" + str(Last_Step_Budget) +"_Total Budget_"+str(Overall_Budget )
        cwd = os.getcwd()
        ...
        last_step_acquisition = Last_Step(model_f=model_f, model_c=None , true_f=f, true_c=None,n_f=m, n_c=n_c, Overall_Budget=Main_Alg_Budget+n_initial_design-1 ,B=Last_Step_Budget,acquisition_optimiser = acq_opt, acquisition_f=acquisition,seed=rep, weight=weight, space=space, path=path)
