import numpy as np
import GPyOpt
from GPyOpt.objective_examples.experiments2d import SRN, BNH, TNK_BNH
import GPy as GPy
...
def TNK_BNH_function_caller_test(rep):
    ...
    np.random.seed(rep)
    folder = "RESULTS"
    subfolder = "TNK_BNH_HV"
    cwd = os.getcwd()
    path = cwd + "/" + folder + "/"+subfolder
    # func2 = dropwave()
    TNK_BNH_func= TNK_BNH(sd=np.sqrt(noise))
    ref_point = TNK_BNH_func.ref_point

    # --- Attributes
    #repeat same objective function to solve a 1 objective problem
    f = MultiObjective([TNK_BNH_func.f1, TNK_BNH_func.f2])
    c = MultiObjective([TNK_BNH_func.c1])
    ...
    space =  GPyOpt.Design_space(space =[{'name': 'var_1', 'type': 'continuous', 'domain': (0, 5.0)},{'name': 'var_2', 'type': 'continuous', 'domain': (0, 3.0)}])#GPyOpt.Design_space(space =[{'name': 'var_1', 'type': 'continuous', 'domain': (0,100)}])#
    n_f = 2
    n_c = 1
    input_d = 2
    m =n_f
    ...
    acq_opt = GPyOpt.optimization.AcquisitionOptimizer(optimizer='lbfgs', space=space, model=model_f, model_c=model_c,NSGA_based=True,analytical_gradient_prediction=False)
    ...
    acquisition = HVI(model=model_f, model_c=model_c , alpha=alpha, space=space, optimizer = acq_opt, utility= U, NSGA_based=True, true_func=f)
    last_step_acquisition = Last_Step(model_f=model_f, model_c=model_c, true_f=f, true_c=c, n_f=n_f, n_c=n_c,
                                      acquisition_optimiser=acq_opt, seed=rep, path=path)
    ...
    X, Y, C, Opportunity_cost = bo.run_optimization(max_iter=max_iter, rep=rep,
...
#TNK_BNH_function_caller_test(rep=1)
