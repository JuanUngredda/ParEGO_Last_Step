...
class HVI(AcquisitionBase):
    ...
    def initial_filtering_hypervolume(self,
        ...
        # generated_mu_values = np.random.random((1000000,2))*(np.array([6,6]) - np.array([1,1]) ) +np.array([1,1])
        # print(generated_mu_values)
    ...
    def hypervolume_computation(self, non_dominated_surface,
        ...
        utility_vector = []
        # print("newy", newy)
        # newy = np.array([1.0,1.5])
        if self.verbose:
            P = self.model.get_Y_values()

            # non_dominated_vectors = np.array(non_dominated_vectors).squeeze()
            # X = initial_design('latin', self.space, 00)
            ...
            mask2 = -non_dominated_surface[:,0][mask] < newy[0]
            mask3 = -non_dominated_surface[:,1][mask] < -1.5
            print(mask2*mask3)
            maskfinal = mask2*mask3
            # plt.scatter(-non_dominated_surface[:,0][mask][maskfinal], -non_dominated_surface[:,1][mask][maskfinal],
            #             c=np.array(utility_vector).reshape(-1)[mask][maskfinal], s=20)
            plt.scatter(-non_dominated_surface[:,0][mask][maskfinal], -non_dominated_surface[:,1][mask][maskfinal],
                        color="grey", s=20)
            plt.scatter(-newy[0], -newy[1], color="black",label="$Y_{UCB}$", s=70)
            plt.scatter(P[0], P[1], color="lightblue", edgecolor="black",label="Sampled $Y$ points", s=70)
            plt.ylabel("max $f_{2}$")
            plt.xlabel("max $f_{1}$")
            plt.title("Objective Space")
            plt.colorbar()
            plt.legend()
            plt.savefig("/home/juan/Documents/repos_data/Last_Step_Preference_Learning/saved_plots/HVI_standard.png", bbox_inches="tight")
            plt.show()
            raise
        else:
            for ndpoint in non_dominated_surface:
