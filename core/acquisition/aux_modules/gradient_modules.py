    ...
    def compute_probability_feasibility_multi_gp(self, x, l=0, gradient_flag = False):
        ...
        if gradient_flag:
            ...
            for m in range(self.model.output_dim):
                ...
                Fz_m = self.compute_probability_feasibility( x, self.model.output[m], mean,  cov, l)

                Fz_m_reformt = np.array(Fz_m).reshape(-1)
                # if np.isnan(Fz_m_reformt).any():
                #
                #     print("Fz", Fz_m, "np.isnan(Fz_m)", np.isnan(Fz_m_reformt))
                #     print("mean_val at nan", np.array(mean).reshape(-1)[np.isnan(Fz_m_reformt)])
                #     print("cov_val at nan", np.array(cov).reshape(-1)[np.isnan(Fz_m_reformt)])
                #     print("Fz with nan")
                #     raise
                Fz.append(Fz_m )
