...
class Last_Step():
    def __init__(self, model_f, model_c, true_f, true_c, n_f, n_c, acquisition_optimiser,acquisition_f,space, Overall_Budget=None,B=1, seed=None, weight = None,path=None):
        self.model = model_f
    ...
    def parEGO_method(self):
        ...
        #training GP
            ...
            print("self.n_last_steps",self.n_last_steps)
            for it in range(self.n_last_steps):
                ...
                if np.sum(feasable_samples)>0:
                    ...
                    self.base_utility = np.min(U_train)
