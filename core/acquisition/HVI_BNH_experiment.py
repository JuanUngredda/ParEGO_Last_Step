import numpy as np
import GPyOpt
from GPyOpt.objective_examples.experiments2d import SRN, BNH
import GPy as GPy
...
def BNH_function_caller_test(rep):
    ...
    # func2 = dropwave()
    BNH_func= BNH(sd=np.sqrt(noise))
    ref_point = BNH_func.ref_point

    # --- Attributes
    #repeat same objective function to solve a 1 objective problem
    f = MultiObjective([BNH_func.f1, BNH_func.f2])
    c = MultiObjective([BNH_func.c1, BNH_func.c2])
    ...
    last_step_acquisition = Last_Step(model_f=model_f, model_c=model_c, true_f=f, true_c=c, n_f=n_f, n_c=n_c,
                                      acquisition_optimiser=acq_opt, acquisition_f=acquisition, seed=rep, space=space,path=path)
...
BNH_function_caller_test(rep=3)
