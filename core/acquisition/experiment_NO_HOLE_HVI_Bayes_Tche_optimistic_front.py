...
from weighted_optimistic_HVI_acquisition import HVI
...
def Bayes_HVI_NO_HOLE_Tche_function_caller_test(rep):
    ...
    for num_queries_idx in range(len(max_number_DMqueries)):
        ...
        for first_query_iteration_element in first_query_iteration[num_queries_idx]:
            ...
            folder = "RESULTS"
            subfolder = "NO_HOLE_weighted_HVI_optimistic_front_Assum_Tche_U_Tche_n_queries_" + str(max_number_DMqueries[num_queries_idx])+"_first_iteration_"+str(first_query_iteration_element)
            cwd = os.getcwd()
