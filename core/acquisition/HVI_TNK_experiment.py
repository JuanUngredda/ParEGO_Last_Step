import numpy as np
import GPyOpt
from GPyOpt.objective_examples.experiments2d import TNK
import GPy as GPy
...
def TNK_function_caller_test(rep):
    ...
    np.random.seed(rep)
    folder = "RESULTS"
    subfolder = "TNK_EGO_utility_LS"
    cwd = os.getcwd()
    path = cwd + "/" + folder + "/"+subfolder

    # func2 = dropwave()
    TNK_func= TNK(sd=np.sqrt(noise))
    ref_point = TNK_func.ref_point

    # --- Attributes
    #repeat same objective function to solve a 1 objective problem
    f = MultiObjective([TNK_func.f1, TNK_func.f2])
    c = MultiObjective([TNK_func.c1, TNK_func.c2])
    ...
    space =  GPyOpt.Design_space(space =[{'name': 'var_1', 'type': 'continuous', 'domain': (0, np.pi)},{'name': 'var_2', 'type': 'continuous', 'domain': (0, np.pi)}])#GPyOpt.Design_space(space =[{'name': 'var_1', 'type': 'continuous', 'domain': (0,100)}])#
    n_f = 2
    n_c = 2
    input_d = 2
    m =n_f
    ...
    acquisition = HVI(model=model_f, model_c=model_c , alpha=alpha, space=space, NSGA_based=True,optimizer = acq_opt, utility= U, true_func=f)

    last_step_acquisition = Last_Step(model_f=model_f, model_c=model_c , true_f=f, true_c=c,n_f=n_f, n_c=n_c, acquisition_optimiser = acq_opt, acquisition_f=acquisition,seed=rep, space=space, path=path)
...
# TNK_function_caller_test(rep=1)
print("ready")
